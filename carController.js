...
const createCar = async (req, res) => {
  try {
    ...
    const car = await Car.create({
      ...
      color: color || '',
      engine: engine || '',
      location: locationObj.lat ? locationObj : { lat: null, lng: null },
      city: city || '',
...
const updateCar = async (req, res) => {
  try {
    ...
    const { locationLat: updLat, locationLng: updLng } = req.body;
    const parsedLat = parseFloat(updLat);
    const parsedLng = parseFloat(updLng);
    if (!isNaN(parsedLat) && !isNaN(parsedLng) && parsedLat >= -90 && parsedLat <= 90 && parsedLng >= -180 && parsedLng <= 180) {
      updateData.location = { lat: parsedLat, lng: parsedLng };
    }
