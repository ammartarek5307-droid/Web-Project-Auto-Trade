...
function createCarCard(car) {
  ...
  const badgeText = car.isUserListed ? (typeof t === 'function' ? t('card.private_seller') : 'Private Seller') : catLabel;
  ...
  return `
    <article class="card" data-car-id="${car.id}">
      <div class="card-img-wrapper">
        <span class="carousel-counter">${Array.isArray(car.images) && car.images.length > 0 ? car.images.length : 1} ${typeof t === 'function' ? t('card.photos') : 'photos'}</span>
        <button class="card-fav-btn ${favored ? 'favorited' : ''}"
                data-car-id="${car.id}"
                aria-label="${favored ? (typeof t === 'function' ? t('car.btn.fav_remove') : 'Remove from Favorites') : (typeof t === 'function' ? t('car.btn.fav_add') : 'Add to Favorites')}"
                title="${favored ? (typeof t === 'function' ? t('car.btn.fav_remove') : 'Remove from Favorites') : (typeof t === 'function' ? t('car.btn.fav_add') : 'Add to Favorites')}">
          <span class="fav-icon">
      ...
      <div class="card-body">
        ...
        ${highestBid ? `<p style="font-size:.75rem;color:var(--color-accent);font-weight:700;margin-bottom:.5rem;">${typeof t === 'function' ? t('bid.highest_bid') : 'Highest bid:'} ${formatEGP(highestBid.amount)}</p>` : ''}
        <div class="card-meta">
          ...
          <div class="card-meta-item">
            ...
            <span>${Number(car.mileage).toLocaleString('en-US')} <span data-i18n="car.mileage">km</span></span>
          </div>
          <div class="card-meta-item">
        ...
        <div class="card-actions">
          <a href="/car-details?id=${car.id}" class="btn btn-primary" id="view-car-${car.id}" style="width: 100%; text-align: center;" data-i18n="general.view_details">View Details</a>
        </div>
      </div>
    </article>
  `;
  ...
  container.querySelectorAll('.card-fav-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      if (isFavorited(car.id)) {
        ...
        showToast(typeof t === 'function' ? t('car.fav.removed') : `Removed "${car.make} ${car.model}" from favorites`, 'info');
      } else {
        saveFavorite(car);
        btn.classList.add('favorited');
        btn.querySelector('.fav-icon svg').setAttribute('fill', 'currentColor');
        showToast(typeof t === 'function' ? t('car.fav.added') : `Added "${car.make} ${car.model}" to favorites!`, 'success');
      }
    });
  });
