...
function injectSettingsSidebar() {
  ...
  const userNameDisplay = user ? user.username : (isGuest ? (typeof t === 'function' ? t('settings.guest') : 'Guest') : (typeof t === 'function' ? t('settings.not_logged') : 'Not Logged In'));
  const userAvatar = user ? user.username.charAt(0).toUpperCase() : (isGuest ? 'G' : '?');
  const showAdminPanelBtn = user && user.role === 'admin' && !window.location.pathname.startsWith('/admin');
  const adminButtonHtml = showAdminPanelBtn ? `<button class="btn btn-outline w-full mb-2" id="settings-admin-panel-btn">⚙ Go to Admin Panel</button>` : '';
    ...
    <div class="settings-body">
      ...
      <div class="settings-account">
        ...
        <div class="settings-user-info">
          <div class="settings-username">${userNameDisplay}</div>
          <div class="settings-role">${user ? (user.role === 'admin' ? (typeof t === 'function' ? t('settings.role.admin') : 'Administrator') : (typeof t === 'function' ? t('settings.role.user') : 'User')) : (isGuest ? (typeof t === 'function' ? t('settings.guest_access') : 'Guest Access') : '')}</div>
        </div>
      </div>
      ...
      <!-- Action Button -->
      ${user 
        ? `${adminButtonHtml}<button class="btn btn-outline w-full mb-2" id="settings-edit-profile-btn" data-i18n="settings.btn.editProfile">Edit Profile</button>
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', () => {
      ...
      closeSettings();
      if (typeof showToast === 'function') showToast(typeof t === 'function' ? t('settings.toast.logged_out') : 'You have been logged out.', 'info');
  ...
  const adminPanelBtn = document.getElementById('settings-admin-panel-btn');
  if (adminPanelBtn) {
...
function openEditProfileModal() {
  ...
  modal.innerHTML = `
    <div class="auth-gate-inner">
      <div class="auth-gate-card">
        <h2 class="auth-gate-title" data-i18n="settings.edit.title">Edit Profile</h2>
        <p class="auth-gate-subtitle" data-i18n="settings.edit.subtitle">Update your account details</p>
        
        <form id="edit-profile-form" class="auth-form" novalidate>
          <div class="auth-field">
            <label for="edit-profile-username" data-i18n="auth.label.username">Username</label>
          ...
          <div class="auth-field">
            <label for="edit-profile-phone" data-i18n="auth.label.phone">Phone Number</label>
          ...
          <div class="auth-field">
            <label for="edit-profile-password" data-i18n="settings.edit.pass_optional">New Password (Optional)</label>
            <div class="password-wrapper">
              <input type="password" id="edit-profile-password" class="form-control" placeholder="Leave blank to keep current" data-i18n-placeholder="settings.edit.pass_ph">
              <button type="button" class="show-pass-btn auth-show-pass" data-target="edit-profile-password" data-i18n="auth.btn.showpass">Show</button>
            </div>
            <span class="form-error" id="err-edit-profile-password"></span>
          </div>
          
          <div style="display: flex; gap: 10px; margin-top: 20px;">
            <button type="button" class="btn btn-outline w-full" id="edit-profile-cancel" data-i18n="settings.edit.cancel">Cancel</button>
            <button type="submit" class="btn btn-primary w-full" id="edit-profile-save" data-i18n="settings.edit.save">Save</button>
          </div>
        </form>
      </div>
    </div>
  ...
  modal.querySelector('.auth-show-pass').addEventListener('click', function() {
    ...
    if (target) {
      ...
      this.textContent = isPass ? (typeof t === 'function' ? t('auth.btn.hidepass') : 'Hide') : (typeof t === 'function' ? t('auth.btn.showpass') : 'Show');
    }
  });
  ...
  document.getElementById('edit-profile-form').addEventListener('submit', async (e) => {
    ...
    if (!phoneInput) {
      ...
      document.getElementById('err-edit-profile-phone').textContent = typeof t === 'function' ? t('auth.err.req_phone') : 'Please enter your phone number.';
      valid = false;
    } else if (typeof validatePhoneInternational === 'function' && !validatePhoneInternational(phoneInput)) {
      document.getElementById('edit-profile-phone').classList.add('field-error');
      document.getElementById('err-edit-profile-phone').textContent = typeof t === 'function' ? t('auth.err.invalid_phone') : 'Please enter a valid phone number.';
      valid = false;
    }
    ...
    saveBtn.textContent = typeof t === 'function' ? t('settings.edit.saving') : 'Saving...';
    ...
    try {
      ...
      if (Object.keys(updateData).length === 0) {
        closeModal();
        if (typeof showToast === 'function') showToast(typeof t === 'function' ? t('settings.toast.no_changes') : 'No changes made.', 'info');
        return;
      }
      ...
      if (data.success) {
        ...
        if (typeof showToast === 'function') showToast(typeof t === 'function' ? t('settings.toast.profile_updated') : 'Profile updated successfully!', 'success');
        ...
        if (typeof showToast === 'function') showToast(data.error || (typeof t === 'function' ? t('settings.toast.update_failed') : 'Failed to update profile.'), 'error');
        saveBtn.disabled = false;
        saveBtn.textContent = typeof t === 'function' ? t('settings.edit.save') : 'Save';
      }
    } catch (err) {
      console.error('Profile update error:', err);
      if (typeof showToast === 'function') showToast(typeof t === 'function' ? t('general.network_err') : 'A network error occurred.', 'error');
      saveBtn.disabled = false;
      saveBtn.textContent = typeof t === 'function' ? t('settings.edit.save') : 'Save';
    }
  });
