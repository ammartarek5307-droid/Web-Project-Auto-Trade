...
async function createUser(username, password, phone) {
  try {
    ...
    return { success: false, error: typeof t === 'function' ? t('auth.err.network') : 'Could not connect to the server. Please try again.' };
  }
}
...
function validatePassword(password) {
  if (password.length < AUTH_CONFIG.minPasswordLength) {
    const msg = typeof t === 'function' ? t('auth.err.pass_length') : 'Password must be at least {0} characters.';
    return msg.replace('{0}', AUTH_CONFIG.minPasswordLength);
  }
  if (!/[A-Z]/.test(password)) {
    return typeof t === 'function' ? t('auth.err.pass_upper') : 'Password must contain at least one uppercase letter.';
  }
  if (!/[0-9]/.test(password)) {
    return typeof t === 'function' ? t('auth.err.pass_number') : 'Password must contain at least one number.';
  }
  if (!/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password)) {
    return typeof t === 'function' ? t('auth.err.pass_special') : 'Password must contain at least one special character.';
  }
  return null;
}
...
function validateUsername(username) {
  if (username.length < 3) return typeof t === 'function' ? t('auth.err.user_length_min') : 'Username must be at least 3 characters.';
  if (username.length > 20) return typeof t === 'function' ? t('auth.err.user_length_max') : 'Username must be 20 characters or less.';
  if (!/^[a-zA-Z0-9_]+$/.test(username)) return typeof t === 'function' ? t('auth.err.user_invalid') : 'Username can only contain letters, numbers, and underscores.';
  return null;
}
  ...
  gate.innerHTML = `
    <div class="auth-gate-inner">
      <div class="auth-gate-card">
        ...
        <h2 class="auth-gate-title" id="auth-gate-title" data-i18n="auth.title.login">Welcome Back</h2>
        <p class="auth-gate-subtitle" id="auth-gate-subtitle" data-i18n="auth.subtitle.login">Sign in to access the marketplace</p>
        ...
        <form id="auth-login-form" class="auth-form" novalidate>
          <div class="auth-field">
            <label for="auth-login-username" data-i18n="auth.label.username">Username</label>
            <input type="text" id="auth-login-username" class="form-control" placeholder="Enter your username" data-i18n-placeholder="auth.ph.username" autocomplete="username" required>
            <span class="form-error" id="err-auth-login-username"></span>
          </div>
          <div class="auth-field">
            <label for="auth-login-password" data-i18n="auth.label.password">Password</label>
            <div class="password-wrapper">
              <input type="password" id="auth-login-password" class="form-control" placeholder="Enter your password" data-i18n-placeholder="auth.ph.password" autocomplete="current-password" required>
              <button type="button" class="show-pass-btn auth-show-pass" data-target="auth-login-password" data-i18n="auth.btn.show">Show</button>
            </div>
            <span class="form-error" id="err-auth-login-password"></span>
          </div>
          <button type="submit" class="btn btn-primary btn-lg w-full auth-submit-btn" id="auth-login-btn" data-i18n="auth.btn.login">
            Log In
          </button>
        </form>
        ...
        <form id="auth-signup-form" class="auth-form" style="display:none;" novalidate>
          <div class="auth-field">
            <label for="auth-signup-username" data-i18n="auth.label.username">Username</label>
            <input type="text" id="auth-signup-username" class="form-control" placeholder="Choose a username" data-i18n-placeholder="auth.ph.choose_username" autocomplete="username" required>
            <span class="form-error" id="err-auth-signup-username"></span>
          </div>
          <div class="auth-field">
            <label for="auth-signup-phone" data-i18n="auth.label.phone">Phone Number</label>
            <input type="tel" id="auth-signup-phone" class="form-control" placeholder="e.g. +20 101 234 5678" data-i18n-placeholder="auth.ph.phone" autocomplete="tel" required>
            <span class="form-error" id="err-auth-signup-phone"></span>
          </div>
          <div class="auth-field">
            <label for="auth-signup-password" data-i18n="auth.label.password">Password</label>
            <div class="password-wrapper">
              <input type="password" id="auth-signup-password" class="form-control" placeholder="Create a strong password" data-i18n-placeholder="auth.ph.create_pass" autocomplete="new-password" required>
              <button type="button" class="show-pass-btn auth-show-pass" data-target="auth-signup-password" data-i18n="auth.btn.show">Show</button>
            </div>
            <span class="form-error" id="err-auth-signup-password"></span>
            <span class="field-hint">${AUTH_CONFIG.passwordRules}</span>
          </div>
          <div class="auth-field">
            <label for="auth-signup-confirm" data-i18n="auth.label.confirm">Confirm Password</label>
            <div class="password-wrapper">
              <input type="password" id="auth-signup-confirm" class="form-control" placeholder="Confirm your password" data-i18n-placeholder="auth.ph.confirm" autocomplete="new-password" required>
              <button type="button" class="show-pass-btn auth-show-pass" data-target="auth-signup-confirm" data-i18n="auth.btn.show">Show</button>
            </div>
            <span class="form-error" id="err-auth-signup-confirm"></span>
          </div>
          <button type="submit" class="btn btn-primary btn-lg w-full auth-submit-btn" id="auth-signup-btn" data-i18n="auth.btn.signup">
            Create Account
          </button>
        </form>
...
function bindAuthGateEvents() {
  ...
  document.querySelectorAll('.auth-show-pass').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (target) {
        ...
        btn.textContent = isPass ? (typeof t === 'function' ? t('auth.btn.hide') : 'Hide') : (typeof t === 'function' ? t('auth.btn.show') : 'Show');
      }
    });
  });
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    if (!username) { setFieldError('auth-login-username', typeof t === 'function' ? t('auth.err.req_user') : 'Please enter your username.'); return; }
    if (!password) { setFieldError('auth-login-password', typeof t === 'function' ? t('auth.err.req_pass') : 'Please enter your password.'); return; }

    const btn = document.getElementById('auth-login-btn');
    btn.disabled = true;
    btn.textContent = typeof t === 'function' ? t('auth.btn.signing_in') : 'Signing in...';
    ...
    if (result.success) {
      setCurrentUser(result.user);
           ...
           // Handle Admin routing — also set admin session so no second login is needed
      if (result.user.role === 'admin') {
           // Set the admin token for client-side API calls
        sessionStorage.setItem('at_admin_token', `${username}:${password}`);
        // Set the server-side admin session
        try {
      ...
      if (typeof showToast === 'function') {
        const welcome = typeof t === 'function' ? t('auth.toast.welcome_back') : 'Welcome back, {0}!';
        showToast(welcome.replace('{0}', result.user.username), 'success');
      }
      ...
      btn.disabled = false;
      btn.textContent = typeof t === 'function' ? t('auth.btn.login') : 'Log In';
    }
  });
  ...
  signupForm.addEventListener('submit', async (e) => {
    ...
    // Phone
    if (!phone) { setFieldError('auth-signup-phone', typeof t === 'function' ? t('auth.err.req_phone') : 'Please enter your phone number.'); valid = false; }
    else if (!validatePhoneInternational(phone)) { setFieldError('auth-signup-phone', typeof t === 'function' ? t('auth.err.invalid_phone') : 'Please enter a valid phone number.'); valid = false; }
    ...
    // Confirm
    if (password !== confirm) { setFieldError('auth-signup-confirm', typeof t === 'function' ? t('auth.err.pass_match') : 'Passwords do not match.'); valid = false; }

    if (!valid) return;

    const btn = document.getElementById('auth-signup-btn');
    btn.disabled = true;
    btn.textContent = typeof t === 'function' ? t('auth.btn.creating_account') : 'Creating account...';
    ...
    if (result.success) {
      setCurrentUser(result.user);
      if (typeof showToast === 'function') {
        const welcomeNew = typeof t === 'function' ? t('auth.toast.welcome_new') : 'Welcome to AutoTrade, {0}!';
        showToast(welcomeNew.replace('{0}', result.user.username), 'success');
      }
      hideAuthGate();
      updateNavForAuth();
    } else {
      showAuthError(result.error);
      btn.disabled = false;
      btn.textContent = typeof t === 'function' ? t('auth.btn.signup') : 'Create Account';
    }
  });
...
function updateNavForAuth() {
  ...
  if (user) {
    ...
    // Add My List link
    const oldMyList = document.getElementById('nav-my-list');
    if (oldMyList) oldMyList.remove();
    const myListLink = document.createElement('a');
    myListLink.href = '/my-listings';
    myListLink.className = 'nav-auth-item';
    myListLink.id = 'nav-my-list';
    myListLink.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="8" y1="6" x2="21" y2="6"></line><line x1="8" y1="12" x2="21" y2="12"></line><line x1="8" y1="18" x2="21" y2="18"></line><line x1="3" y1="6" x2="3.01" y2="6"></line><line x1="3" y1="12" x2="3.01" y2="12"></line><line x1="3" y1="18" x2="3.01" y2="18"></line></svg> <span data-i18n="nav.mylistings">My Listings</span>';

    // Insert before settings if settings is there
    const settingsBtn = document.getElementById('nav-settings-btn');
    if (settingsBtn) {
      navLinks.insertBefore(inboxLink, settingsBtn);
      navLinks.insertBefore(myListLink, settingsBtn);
    } else {
      navLinks.appendChild(inboxLink);
      navLinks.appendChild(myListLink);
    }
  }

  // Re-apply translations after nav elements added
