...
function openMessageSellerModal(carId, carName) {
  ...
  // Find the car in the loaded cache
  const car = getAllCars().find(c => String(c.id) === String(carId));
  if (!car) {
    showToast(typeof t === 'function' ? t('msg.err.not_found') : 'Listing not found.', 'error');
    return;
  }

  // sellerId is the real MongoDB user ID of the seller
  const sellerUserId = car.sellerId;

  if (!sellerUserId) {
    showToast(typeof t === 'function' ? t('msg.err.no_seller') : 'Unable to contact seller — seller information is missing.', 'warning');
    return;
  }

  if (String(currentUser.id) === String(sellerUserId)) {
    showToast(typeof t === 'function' ? t('msg.err.self') : 'You cannot message yourself.', 'warning');
    return;
  }

  // Open the message modal with seller's real user ID as recipientId
...
async function showMessageModal(convo, currentUser, recipientId, carName, carId = null) {
  ...
  modal.innerHTML = `
    <div class="modal-box msg-modal-box">
      ...
      <div class="msg-modal-header">
        ...
        <div>
          <h3>${recipientName}</h3>
          ${carName ? `<p class="msg-modal-car">${typeof t === 'function' ? t('msg.about') : 'About'}: ${carName}</p>` : ''}
        </div>
      </div>

      <div class="msg-modal-body" id="msg-modal-body">
        ${convoMessages.length === 0 ? `<p class="msg-empty" data-i18n="msg.empty.convo">${typeof t === 'function' ? t('msg.empty.convo') : 'Start a conversation with the seller'}</p>` :
          convoMessages.map(m => `
            <div class="msg-bubble ${m.senderId === currentUser.id ? 'msg-sent' : 'msg-received'}">
              <p>${escapeHtml(m.content)}</p>
              <span class="msg-time">${formatMsgTime(m.createdAt)}${m.senderId === currentUser.id && m.readAt ? ' · ' + (typeof t === 'function' ? t('msg.read') : 'Read') : ''}</span>
            </div>
          `).join('')}
      </div>

      <form class="msg-modal-input" id="msg-send-form">
        <input type="text" id="msg-input" class="form-control" placeholder="Type a message..." data-i18n-placeholder="msg.ph.type" autocomplete="off" maxlength="500" required>
  ...
  document.getElementById('msg-send-form').addEventListener('submit', async e => {
    ...
    if (data.success) {
      ...
      bubble.innerHTML = `<p>${escapeHtml(text)}</p><span class="msg-time">${typeof t === 'function' ? t('msg.just_now') : 'Just now'}</span>`;
      body.appendChild(bubble);
      body.scrollTop = body.scrollHeight;
    } else {
      showToast(data.error || (typeof t === 'function' ? t('msg.err.send_failed') : 'Failed to send message.'), 'error');
    }
  });
...
function formatMsgTime(isoStr) {
  ...
  if (diff < 3600000) return Math.floor(diff / 60000) + ' ' + (typeof t === 'function' ? t('msg.time.mins') : 'm ago');
  if (diff < 86400000) return Math.floor(diff / 3600000) + ' ' + (typeof t === 'function' ? t('msg.time.hours') : 'h ago');
  ...
  if (convos.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <h3 data-i18n="msg.empty.title">No conversations yet</h3>
        <p data-i18n="msg.empty.desc">When you message a seller, your conversations will appear here.</p>
        <a href="/categories?type=all" class="btn btn-primary btn-lg" data-i18n="nav.browse">Browse Cars</a>
      </div>
    `;
    return;
  }
    ...
    <div class="inbox-layout">
      <div class="inbox-sidebar" id="inbox-sidebar">
        <div class="inbox-sidebar-header">
          <h2 data-i18n="msg.title">Messages</h2>
          <span class="inbox-count">${convos.length} conversation${convos.length !== 1 ? 's' : ''}</span>
        </div>
        <div class="inbox-search">
          <input type="text" class="form-control" placeholder="Search conversations..." data-i18n-placeholder="msg.ph.search" id="inbox-search-input">
        </div>
        <div class="inbox-list" id="inbox-list">
          ${convos.map(c => `
            <div class="inbox-item ${c.unreadCount > 0 ? 'unread' : ''}" data-convo-id="${c._id}" role="button" tabindex="0">
              <div class="inbox-item-avatar">${(c.otherUser && c.otherUser.username ? c.otherUser.username : 'U').charAt(0).toUpperCase()}</div>
              <div class="inbox-item-info">
                <div class="inbox-item-name">
                  ${c.otherUser && c.otherUser.username ? c.otherUser.username : 'User'}
      ...
      <div class="inbox-chat" id="inbox-chat">
        <div class="inbox-chat-empty">
          ...
          <p data-i18n="msg.select.convo">Select a conversation to view messages</p>
        </div>
      </div>
    </div>
  `;
...
async function openInboxConversation(convoId, currentUser) {
  ...
  chatArea.innerHTML = `<div class="inbox-chat-empty"><p data-i18n="msg.loading">${typeof t === 'function' ? t('msg.loading') : 'Loading messages...'}</p></div>`;

  const data = await fetchApi(`/api/messages/conversations/${convoId}`);
  if (!data.success) {
    chatArea.innerHTML = `<div class="inbox-chat-empty"><p data-i18n="msg.error">${typeof t === 'function' ? t('msg.error') : 'Error loading messages'}</p></div>`;
    return;
  }
  ...
  chatArea.innerHTML = `
    ...
    <div class="inbox-chat-messages" id="inbox-chat-messages">
      ${convoMessages.length === 0 ? `<p class="msg-empty" data-i18n="msg.empty.chat">${typeof t === 'function' ? t('msg.empty.chat') : 'No messages yet. Start the conversation!'}</p>` :
        convoMessages.map(m => `
          <div class="msg-bubble ${m.senderId === currentUser.id ? 'msg-sent' : 'msg-received'}">
            <p>${escapeHtml(m.content)}</p>
            <span class="msg-time">${formatMsgTime(m.createdAt)}${m.senderId === currentUser.id && m.readAt ? ' · ' + (typeof t === 'function' ? t('msg.read') : 'Read') : ''}</span>
    ...
    <form class="inbox-chat-input" id="inbox-chat-form">
      <input type="text" id="inbox-msg-input" class="form-control" placeholder="Type a message..." data-i18n-placeholder="msg.ph.type" autocomplete="off" maxlength="500" required>
  ...
  document.getElementById('inbox-chat-back')?.addEventListener('click', () => {
    chatArea.innerHTML = `<div class="inbox-chat-empty">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="opacity:.3"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
      <p data-i18n="msg.select.convo">${typeof t === 'function' ? t('msg.select.convo') : 'Select a conversation'}</p>
    </div>`;
    document.querySelectorAll('.inbox-item').forEach(i => i.classList.remove('active'));
