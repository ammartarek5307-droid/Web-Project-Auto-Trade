...
// ── Place a bid ──
function placeBid(car, bidderId, bidderName, amount) {
  const bids = getBids();
  ...
  if (currentUser) {
    if (String(car.sellerId) === String(currentUser.id)) {
...
function renderBiddingSection(car, container) {
  ...
  if (form && currentUser) {
    form.addEventListener('submit', (e) => {
      ...
      const result = placeBid(car, currentUser.id, currentUser.username, amount);
