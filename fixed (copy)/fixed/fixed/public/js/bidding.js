...
function updateBidUI(carId) {
  ...
  if (highestDisplay) {
    highestDisplay.textContent = highestBid ? formatEGP(highestBid.amount) : 'No bids yet';
  }
  ...
  if (form) {
    ...
    if (hint) hint.textContent = `Minimum bid: ${formatEGP(minBid)} (${formatEGP(BID_CONFIG.minIncrement)} increment)`;
  }
}
